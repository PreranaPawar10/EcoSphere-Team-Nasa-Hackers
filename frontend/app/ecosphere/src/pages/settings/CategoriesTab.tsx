...
export function CategoriesTab() {
  ...
  const { data, isLoading, isError } = useCategories(debouncedSearch, page);
      ...
      <div className="bg-slate-900/50 rounded-lg border border-slate-700/50 overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-700/50">
            ...
            {isError && (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-red-400">Failed to load data</td>
              </tr>
            )}
            {data?.items.length === 0 && (
              <tr>
                <td colSpan={4} className="px-6 py-8 text-center text-slate-400">No categories found</td>
              </tr>
            )}
